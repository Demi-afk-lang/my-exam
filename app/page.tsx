...
export default function ExamPage() {
  ...
  return (
    <main className="min-h-screen bg-[#020617] font-sans flex items-center justify-center text-white" dir="rtl">
      <AnimatePresence mode="wait">
        {stage === 'landing' && (
          <motion.div key="landing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 1.1 }} className="text-center p-6">
              ...
              <button onClick={() => setStage('quiz')} className="bg-white text-black px-16 py-5 rounded-full text-2xl font-black hover:bg-yellow-400 transition-colors">دخول الامتحان</button>
          </motion.div>
        )}

        {stage === 'quiz' && (
          <motion.div key="quiz" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-3xl p-6">
                ...
                <div key={q.id} className="bg-[#0f172a] p-8 rounded-[35px] border border-slate-800">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {q.options.map((option, idx) => {
                      ...
                      return (
                        <button key={option} onClick={() => handleSelect(q.id, letter)} className={`p-4 rounded-2xl border-2 text-right font-bold flex items-center ${style}`}>
            ...
            {Object.keys(answers).length === fastQuestions.length && (
              <button onClick={() => setStage('result')} className="w-full mt-10 bg-yellow-500 text-black py-5 rounded-3xl font-black text-2xl">عرض النتيجة</button>
            )}
          </motion.div>
        )}

        {stage === 'result' && (
          <motion.div key="result" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center p-8 bg-[#0f172a] border border-slate-800 rounded-[50px] max-w-md w-full">
              <h2 className="text-slate-500 text-sm font-bold mb-4 uppercase">درجتك النهائية هي</h2>
              <div className="text-8xl font-black mb-4 text-white">{percentage}%</div>
              <p className="text-2xl font-bold mb-8 text-yellow-400">{getFeedback()}</p>
              <button onClick={() => setStage('goodbye')} className="w-full py-4 bg-white/5 rounded-2xl text-slate-300 font-bold">إغلاق الامتحان</button>
          </motion.div>
        )}
        ...
        {stage === 'goodbye' && (
          <motion.div key="goodbye" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center p-10">
              ...
              <p className="text-slate-400 text-xl mb-2">تم تسجيل خروجك بأمان</p>
              <p className="text-blue-500 font-bold italic uppercase">SEE YOU IN THE NEXT CHALLENGE</p>
          </motion.div>
        )}
      </AnimatePresence>
    </main>
